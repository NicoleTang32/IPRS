...
IPRS_stratification_population<-function(Outcome,Environment,IPRS_Analysis_Data){
  ...
  Preva_IPRS$preva[1]<-sum(IPRS_low5$Y==1)/length(IPRS_low5$Y)
  ...
  Preva_IPRS$preva[2]<-sum(IPRS_mediate$Y==1)/length(IPRS_mediate$Y)
  ...
  Preva_IPRS$preva[3]<-sum(IPRS_top5$Y==1)/length(IPRS_top5$Y)
